...
import { adminGetConversations, adminGetMessages, adminSendMessage } from "../../api";
...
export default function AdminChat() {
  ...
  const send = async (e) => {
    ...
    if (!t || !active) return;
    setText("");
    setMessages((m) => [...m, { _id: "tmp" + Date.now(), sender: "admin", text: t, createdAt: new Date().toISOString() }]);
    try {
      await adminSendMessage(active.clientId, t);
      loadMessages(active.clientId);
      loadConvos();
