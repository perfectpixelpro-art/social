...
export default function Managers() {
  ...
  const create = async (e) => {
    ...
    try {
      await adminCreateManager({ ...form, clientIds: form.role === "manager" ? pickClients : [] });
      setStatus({ loading: false, ok: true, msg: `${form.role === "writer" ? "Writer" : "Manager"} created!` });
  ...
  return (
    <main className="h-full overflow-y-auto p-6 mq450:p-4">
      <h1 className="m-0 text-[#0b1f44] font-bold" style={{ fontSize: "clamp(22px, 3vw, 28px)" }}>{isManager ? "Assign Clients" : "Team & Clients"}</h1>
      <p className="m-0 mt-1 mb-6 text-[14px] text-[#7a8499] font-medium">{isManager ? "Assign clients to managers." : "Create managers, assign clients, and manage their access."}</p>

      {/* Add a manager (admin only) */}
      {!isManager && (
      <section className="rounded-[16px] border border-[#eef1f6] p-6 mq450:p-4 mb-6 max-w-[640px]">
        <h2 className="m-0 text-[16px] font-bold text-[#0b1f44] mb-4">Add a Team Member</h2>
        <form onSubmit={create} className="flex flex-col gap-4">
          {/* role */}
          <div className="flex gap-2">
            {[["manager", "Manager"], ["writer", "Writer"]].map(([val, label]) => (
              <button key={val} type="button" onClick={() => setForm((f) => ({ ...f, role: val }))}
                className={`flex-1 rounded-[10px] border px-4 py-2.5 text-[14px] font-bold cursor-pointer ${form.role === val ? "border-[#013186] bg-[#eaf1ff] text-[#013186]" : "border-[#e3e9f5] text-[#5b6472] hover:bg-[#f5f7fb]"}`}>
                {label}
              </button>
            ))}
          </div>
          ...
          {/* assign clients on create — managers only */}
          <div className={form.role === "writer" ? "hidden" : ""}>
          ...
          <button type="submit" disabled={status.loading} className="h-[46px] rounded-[10px] bg-[#013186] text-white font-bold text-[15px] hover:bg-[#012270] cursor-pointer disabled:opacity-60">
            {status.loading ? "Creating..." : `Create ${form.role === "writer" ? "Writer" : "Manager"}`}
          </button>
      ...
      <h2 className="m-0 mb-3 text-[16px] font-bold text-[#0b1f44]">Team ({managers.length})</h2>
        ...
        {managers.map((m) => (
          <section key={m.id} className="rounded-[16px] border border-[#eef1f6] p-5 mq450:p-4">
            <div className="flex items-center gap-3">
              <span className="w-10 h-10 rounded-full text-white flex items-center justify-center text-[14px] font-bold" style={{ background: m.role === "writer" ? "#0ea5e9" : "#7c3aed" }}>{initialsOf(m.name)}</span>
              <div className="min-w-0 flex-1">
                <p className="m-0 text-[15px] font-bold text-[#0b1f44] truncate">{m.name}
                  <span className="ml-2 text-[10px] font-bold rounded-full px-2 py-0.5 align-middle" style={{ background: m.role === "writer" ? "#e0f2fe" : "#f0eafe", color: m.role === "writer" ? "#0369a1" : "#7c3aed" }}>{m.role === "writer" ? "Writer" : "Manager"}</span>
                </p>
                <p className="m-0 text-[12px] text-[#9aa3b2] truncate">{m.email}{m.role === "manager" ? ` · ${m.clientCount} client${m.clientCount !== 1 ? "s" : ""}` : ""}</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {m.role !== "writer" && <button onClick={() => setExpanded(expanded === m.id ? null : m.id)} className="text-[13px] font-semibold text-[#013186] border border-[#cfe0ff] rounded-[8px] px-3 py-2 hover:bg-[#f0f6ff] cursor-pointer">Assign clients</button>}
              {!isManager && <button onClick={() => resetPw(m)} className="text-[13px] font-semibold text-[#5b6472] border border-[#e3e9f5] rounded-[8px] px-3 py-2 hover:bg-[#f5f7fb] cursor-pointer">Reset password</button>}
              {!isManager && <button onClick={() => removeAccess(m)} className="text-[13px] font-semibold text-[#dc2626] border border-[#f3c0c0] rounded-[8px] px-3 py-2 hover:bg-[#fdf2f2] cursor-pointer">Remove access</button>}
            </div>
