...
const PostCard = ({ src, alt, showLeftArrow, showRightArrow, onPrev, onNext }) => (
  <div style={{ position: "relative", width: "300px", flexShrink: 0 }}>
    {showLeftArrow && (
      <button onClick={onPrev} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", zIndex: 30, width: 32, height: 32, borderRadius: "50%", background: "#fff", border: "1px solid rgba(1,49,134,0.15)", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.12)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#013186" strokeWidth="2.5"><polyline points="15 18 9 12 15 6"/></svg>
      </button>
    )}
    {showRightArrow && (
      <button onClick={onNext} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", zIndex: 30, width: 32, height: 32, borderRadius: "50%", background: "#fff", border: "1px solid rgba(1,49,134,0.15)", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.12)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#013186" strokeWidth="2.5"><polyline points="9 18 15 12 9 6"/></svg>
      </button>
    )}
    <div style={{ borderRadius: 14, overflow: "hidden", background: "#fff", boxShadow: "0 4px 24px rgba(1,49,134,0.10)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          <div style={{ width: 26, height: 26, borderRadius: "50%", background: "#013186", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ color: "#fff", fontSize: 9, fontWeight: 700 }}>S</span>
          </div>
          <span style={{ fontSize: 11, fontWeight: 600, color: "#222" }}>thesocial99</span>
        </div>
        <span style={{ color: "#aaa", fontSize: 15, letterSpacing: 2 }}>···</span>
      </div>
...
const FrameComponent111 = ({ className = "" }) => {
  ...
  return (
    <main className={`font-[Montserrat] ${className}`} style={{ width: "100%", overflow: "hidden" }}>
      ...
      >
        {/* FAR LEFT */}
        <div className="mq1125:hidden flex-shrink-0" style={{ marginTop: "5%" }}>
          <PostCard {...POST_SLIDES[gi(-2)]} showLeftArrow onPrev={prev} />
        </div>

        {/* LEFT */}
        <div className="mq450:hidden flex-shrink-0" style={{ marginTop: "5%" }}>
