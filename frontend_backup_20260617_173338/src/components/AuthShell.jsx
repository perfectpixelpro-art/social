...
export default function AuthShell({ children, wide = false }) {
  return (
    <div className="w-full min-h-screen flex font-[Montserrat] bg-[#021b4a]">
      ...
      {/* Right form panel */}
      <div className={`${wide ? "w-[62%] mq1125:w-full" : "w-1/2 mq800:w-full"} bg-white flex items-center justify-center px-10 mq450:px-6 py-16`}>
        <div className={`w-full ${wide ? "max-w-[820px]" : "max-w-[440px]"}`}>
          {/* mobile logo */}
          <Link to="/" className={`${wide ? "hidden mq1125:flex" : "hidden mq800:flex"} mb-10 justify-center`}>
