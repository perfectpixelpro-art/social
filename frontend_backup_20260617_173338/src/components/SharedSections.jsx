...
const CtaButton = ({ label = "Start for $99", href = "/checkout?plan=Starter" }) => (
...
export function BrandShine({
  ...
  return (
    <div className="w-full px-[100px] mq800:px-10 mq450:px-5 py-20 flex justify-center font-[Montserrat]">
      <div className="relative w-full max-w-[1100px] flex items-center justify-center py-10 xl:py-0 xl:aspect-[1100/620]">
        {/* Concentric circles — only on wide desktop where the ring fits */}
        <svg className="absolute inset-0 w-full h-full pointer-events-none hidden xl:block" viewBox="0 0 1100 620" preserveAspectRatio="xMidYMid meet">
          {[110, 200, 290, 380].map((r) => (
            <circle key={r} cx="550" cy="310" r={r} fill="none" stroke="rgba(1,49,134,0.06)" strokeWidth="1.5" />
          ))}
        </svg>

        {/* Floating badges — only on wide desktop (xl+) so they never clip */}
        {platforms.map((b) => (
          <div
            key={b.label}
            className="absolute hidden xl:flex items-center gap-2 bg-white rounded-full shadow-[0_4px_20px_rgba(1,49,134,0.10)] px-4 py-2"
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          {/* Wrapped chips — shown on everything below xl (tablet/laptop/mobile) */}
          <div className="flex xl:hidden flex-wrap items-center justify-center gap-2 mt-8 max-w-[640px]">
            {platforms.map((b) => (
...
// Maps the displayed package order (Basic/Standard/Premium) to the canonical
// plan names used by the checkout + Stripe (Starter/Growth/Pro).
const PLAN_BY_INDEX = ["Starter", "Growth", "Pro"];

export function PlansThatFit({ data = plans, tab = "Marketing", subtitle = "Flexible plans designed to deliver value and results." }) {
  const [active, setActive] = useState(0);
  const checkoutHref = `/checkout?tab=${encodeURIComponent(tab)}&plan=${encodeURIComponent(PLAN_BY_INDEX[active] || "Starter")}`;
  return (
    <div className="w-full px-[100px] mq800:px-10 mq450:px-5 py-16 flex flex-col items-center font-[Montserrat]">
      ...
      <div className="w-full max-w-[1500px] flex items-stretch gap-8 mq1125:flex-col">
        ...
        <div className="flex-1 rounded-[18px] border border-[rgba(1,49,134,0.12)] p-10 mq450:p-6 flex flex-col">
          ...
          <a href={checkoutHref} className="self-end mt-8 flex items-center gap-2 no-underline border border-[rgba(1,49,134,0.07)] bg-[rgba(188,214,255,0.37)] hover:bg-[rgba(188,214,255,0.6)] transition-colors h-[48px] rounded-[25.5px] pl-6 pr-[6px]">
    ...
    <div className="w-full px-[100px] mq800:px-10 mq450:px-5 py-16 flex justify-center font-[Montserrat]">
      <div className="w-full max-w-[1600px] rounded-[24px] bg-[#f4f8ff] border border-[rgba(1,49,134,0.08)] p-12 mq800:p-8 mq450:p-5 flex items-center gap-10 mq1125:flex-col">
        ...
        <div className="flex-1 w-full flex flex-col items-start">
          ...
          <a href="/checkout?plan=Starter" className="mt-8 flex items-center gap-2 no-underline border border-[rgba(1,49,134,0.07)] bg-[rgba(188,214,255,0.37)] hover:bg-[rgba(188,214,255,0.6)] transition-colors h-[50px] rounded-[25.5px] pl-6 pr-[6px]">
